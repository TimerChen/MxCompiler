/*
	Coded by timemachine on 18-6-2
	MxCompiler.CodeGen.IRTranslator
*/
...
public class IRTranslator implements IRVisitor
{
	...
	public IRTranslator(List<InsIR> insIRList, List<StringLitIR> litIRlist, List<GlobalVarIR>globalVarIRS)
	{
	...
	private List<String> getCodeStr()
	{
		...
		boolean oldCRC = Global.checkRegColored;
		Global.checkRegColored = false;
		//Prefix
	...
	private List<String> visitInsIR(List<InsIR> list)
	{
		...
		{
			i.accept(this);
			ret.add("["+i.getClass().getSimpleName()+"]");
	...
	public void visit(SpecialIR node)
	{
		...
		list.add(CODE_PREFIX+"special\t"+node.type());
